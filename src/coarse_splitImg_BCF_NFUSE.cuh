#ifndef COARSE_MATRIX_KERNEL_IM2COL_SPLITIMG_BCF_NFUSE_CUH
#define COARSE_MATRIX_KERNEL_IM2COL_SPLITIMG_BCF_NFUSE_CUH
...
__global__ void CoarseMatrixKernelIm2colSplitImgBCFNfuseKernel(
	XFLOAT *g_eulers,
	XFLOAT *trans_x,
	XFLOAT *trans_y,
	XFLOAT *g_real,
...
struct CoarseMatrixKernelIm2colSplitImgBCFNfuse {
  ...
  void run(XFLOAT *g_eulers, 
    ...
    if constexpr (!std::is_same_v<TParams, void>) {
      ...
      CoarseMatrixKernelIm2colSplitImgBCFNfuseKernel<TParams>
	  <<<grid, block, 0, stream>>>(
      ...
      dispatch_TParams([=](auto dummy) {
		...
		CoarseMatrixKernelIm2colSplitImgBCFNfuseKernel<SelectedTParams><<<grid, block, 0, stream>>>(
            g_eulers, trans_x, trans_y, g_real, g_imag, projector, g_corr,
            g_diff2s, g_diff2s_dest, translation_num_, orientation_num_, image_size_, full_image_size_, g_coor_xy);
      });
    }
...
/*=============================== FOR IMAGE BLOCK ==============================*/
        ...
        while (scheduler.get_current_work_next_k_cycle(k_cycle)) {
			...
			if (k_cycle > scheduler.get_current_work_k_cycle_start()) {
				...
				orientation_matrix_handler.construct_orientation_matrix_bank_conflict_free(
...
#endif // COARSE_MATRIX_KERNEL_IM2COL_SPLITIMG_BCF_NFUSE_CUH