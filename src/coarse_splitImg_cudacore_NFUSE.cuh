#ifndef COARSE_MATRIX_KERNEL_IM2COL_SPLITIMG_CUDACORE_NFUSE_CUH
#define COARSE_MATRIX_KERNEL_IM2COL_SPLITIMG_CUDACORE_NFUSE_CUH
...
__global__ void CoarseMatrixKernelCudaCoreIm2colSplitImgNfuseKernel(
	XFLOAT *g_eulers,
	XFLOAT *trans_x,
	XFLOAT *trans_y,
	XFLOAT *g_real,
...
struct CoarseMatrixKernelCudaCoreIm2colSplitImgNfuse {
  ...
  void run(XFLOAT *g_eulers, 
    ...
    if constexpr (!std::is_same_v<TParams, void>) {
      ...
      CoarseMatrixKernelCudaCoreIm2colSplitImgNfuseKernel<TParams>
	  <<<grid, block, 0, stream>>>(
      ...
      dispatch_TParams([=](auto dummy) {
		...
		CoarseMatrixKernelCudaCoreIm2colSplitImgNfuseKernel<SelectedTParams><<<grid, block, 0, stream>>>(
            g_eulers, trans_x, trans_y, g_real, g_imag, projector, g_corr,
            g_diff2s, g_diff2s_dest, translation_num_, orientation_num_, image_size_, full_image_size_, g_coor_xy);
      });
    }
...
__global__ void CoarseMatrixKernelCudaCoreIm2colSplitImgNfuseKernel(
	...
	using TransMatLayout = SharedMemoryNoSwizzle<float, TParams::kTransBlockSize, 2 * TParams::kImgBlockSize, 0>;
	using TransRealMatLayout = SharedMemoryNoSwizzle<float, TParams::kTransBlockSize, TParams::kImgBlockSize, 0>;
	using TransImagMatLayout = SharedMemoryNoSwizzle<float, TParams::kTransBlockSize, TParams::kImgBlockSize, TParams::kImgBlockSize>;
	
	using OrientMatLayout = SharedMemoryNoSwizzle<float, TParams::kOrientBlockSize, 2 * TParams::kImgBlockSize, 0>;
	using OrientRealMatLayout = SharedMemoryNoSwizzle<float, TParams::kOrientBlockSize, TParams::kImgBlockSize, 0>;
	using OrientImagMatLayout = SharedMemoryNoSwizzle<float, TParams::kOrientBlockSize, TParams::kImgBlockSize, TParams::kImgBlockSize>;
...
/*=============================== FOR IMAGE BLOCK ==============================*/
        ...
        while (scheduler.get_current_work_next_k_cycle(k_cycle)) {
			...
			if (k_cycle > scheduler.get_current_work_k_cycle_start()) {
				...
				load_coord_xy(img_block_idx, s_corr_div_2[k_cycle_mod2], 
							  s_coor_x[k_cycle_mod2], s_coor_y[k_cycle_mod2], 
							  s_fcoor_xy[k_cycle_mod2], s_img_real_imag[k_cycle_mod2]);
			}
			__syncthreads();
			...
			if (k_cycle > scheduler.get_current_work_k_cycle_start()) {
				...
				/*=============================== COMPUTE CROSS TERM ==============================*/
				cuda_core_mma_fp32<TransMatLayout, OrientMatLayout, 
				TParams::kTransBlockSize, TParams::kOrientBlockSize, 2 * TParams::kImgBlockSize,
				TParams::kWarpTransTileSize, TParams::kWarpOrientTileSize, 2 * TParams::kWarpImgTileSize,
				TParams::kMmaTransTileSize, TParams::kMmaOrientTileSize, TParams::kMmaImgTileSize>(
...
#endif // COARSE_MATRIX_KERNEL_IM2COL_SPLITIMG_CUDACORE_NFUSE_CUH