...
struct TranslationMatrixHandler {
	...
	__device__ __forceinline__
	void construct_translation_matrix_bank_conflict_free(
		// out
		TransMatLayout s_trans_mat_block_swizzle,
		TransRealMatLayout s_trans_real_mat_block_swizzle,
		TransImagMatLayout s_trans_imag_mat_block_swizzle,
		float* s_trans_pow2_accumulator, // size: kTransBlockSize
		// in
		...
		*reinterpret_cast<float2*>(&reg_fcoor_xy[0])
		...
		for (int i = 0; i < kTMNumTransWarpTilePerWarp; i ++) {
			...
			*reinterpret_cast<float2*>(&reg_trans_xy[i][0]) 
			...
			XFLOAT reg_trans_mat_v[2];
			reg_trans_mat_v[0] = -2 * reg_trans_real_imag[i][0] * reg_corr_div_2;
			reg_trans_mat_v[1] = -2 * reg_trans_real_imag[i][1] * reg_corr_div_2;

			int s_img_idx1, s_img_idx2;
			// less than 16 threads
			if ((lane_id & 0x10) == 0) {
				s_img_idx1 = s_img_idx;
				s_img_idx2 = s_img_idx + 16;
			} else {
				s_img_idx1 = s_img_idx + 16;
				s_img_idx2 = s_img_idx;
				// swap
				XFLOAT tmp = reg_trans_mat_v[0];
				reg_trans_mat_v[0] = reg_trans_mat_v[1];
				reg_trans_mat_v[1] = tmp;
			}

			s_trans_mat_block_swizzle(s_trans_idx, s_img_idx1) = reg_trans_mat_v[0];
			s_trans_mat_block_swizzle(s_trans_idx, s_img_idx2) = reg_trans_mat_v[1];
