#ifndef COARSE_MATRIX_KERNEL_IM2COL_SPLITIMG_BCF_PMO_TMDB_NFUSE_CUH
#define COARSE_MATRIX_KERNEL_IM2COL_SPLITIMG_BCF_PMO_TMDB_NFUSE_CUH
...
__global__ void CoarseMatrixKernelIm2colSplitImgBCFProjOverlapTransDBNfuseKernel(
	XFLOAT *g_eulers,
	XFLOAT *trans_x,
	XFLOAT *trans_y,
	XFLOAT *g_real,
...
struct CoarseMatrixKernelIm2colSplitImgBCFProjOverlapTransDBNfuse {
  ...
  CoarseMatrixKernelIm2colSplitImgBCFProjOverlapTransDBNfuse(
      int translation_num, int orientation_num, int image_size, int full_image_size, int sm_num = 108)
  ...
  size_t get_workspace_size_bytes() const {
    if constexpr (!std::is_same_v<TParams, void>) {
      ...
      return scheduler.get_workspace_size_bytes();
    } else {
      return dispatch_TParams([this](auto dummy) -> size_t {
  ...
  void run(XFLOAT *g_eulers, 
    ...
    if constexpr (!std::is_same_v<TParams, void>) {
      ...
      CoarseMatrixKernelIm2colSplitImgBCFProjOverlapTransDBNfuseKernel<TParams>
	  <<<grid, block, 0, stream>>>(
          g_eulers, trans_x, trans_y, g_real, g_imag, projector, g_corr,
          g_diff2s, g_diff2s_dest, translation_num_, orientation_num_, image_size_, full_image_size_, g_coor_xy, work_space);
    } else {
      dispatch_TParams([=](auto dummy) {
		...
		CoarseMatrixKernelIm2colSplitImgBCFProjOverlapTransDBNfuseKernel<SelectedTParams><<<grid, block, 0, stream>>>(
            g_eulers, trans_x, trans_y, g_real, g_imag, projector, g_corr,
            g_diff2s, g_diff2s_dest, translation_num_, orientation_num_, image_size_, full_image_size_, g_coor_xy, work_space);
      });
    }
  ...
  void print_workspace_buffer(uint32_t* workspace) {
	if constexpr (!std::is_same_v<TParams, void>) {
	  CoarseScheduler<TParams::kTransBlockSize,
					  ...
					  CoarseSchedulerStrategy::InterleavedSplitK,
					  2> scheduler(translation_num_, orientation_num_, image_size_, 1, 0);
	  scheduler.print_workspace_buffer(workspace);
	} else {
	  dispatch_TParams([this, workspace](auto dummy) {
		using SelectedTParams = typename decltype(dummy)::type;
		CoarseScheduler<SelectedTParams::kTransBlockSize,
						SelectedTParams::kOrientBlockSize,
						SelectedTParams::kImgBlockSize,
...
// trans pipe, need extral workspace
template<typename TParams>
__launch_bounds__(128, BLOCKS_PER_SM)
__global__ void CoarseMatrixKernelIm2colSplitImgBCFProjOverlapTransDBNfuseKernel(
	XFLOAT *g_eulers,
	...
	__align__(16) __shared__ XFLOAT s_trans_mat_block[2 * 2 * TParams::kTransBlockSize * TParams::kImgBlockSize]; // double buffer
	...
	TransMatLayout s_trans_mat_block_swizzle[2] = {
		TransMatLayout(s_trans_mat_block),
		TransMatLayout(s_trans_mat_block + 2 * TParams::kTransBlockSize * TParams::kImgBlockSize)
	};

	TransRealMatLayout s_trans_real_mat_block_swizzle[2] = {
		TransRealMatLayout(s_trans_mat_block),
		TransRealMatLayout(s_trans_mat_block + 2 * TParams::kTransBlockSize * TParams::kImgBlockSize)
	};

	TransImagMatLayout s_trans_imag_mat_block_swizzle[2] = {
		TransImagMatLayout(s_trans_mat_block),
		TransImagMatLayout(s_trans_mat_block + 2 * TParams::kTransBlockSize * TParams::kImgBlockSize)
	};
	...
	// double buffer for s_corr_div_2, s_fcoor_xy, s_img_real_imag
	...
	// reduce buffer
	__align__(16) __shared__ XFLOAT s_trans_pow2_accumulator[TParams::kBlockSize];
	__align__(16) __shared__ XFLOAT s_orient_pow2_accumulator[TParams::kOrientBlockSize];

	// used for dummy store shared
	__align__(16) __shared__ uint32_t s_trans_mat_buffer[32];
	...
	TranslationMatrixHandler<TParams::kTransBlockSize,
							 ...
							 TransRealMatLayout,
							 TransImagMatLayout>
							 translation_matrix_handler(image_size, translation_num, &s_trans_mat_buffer[0]);		
	...
	auto load_coord_xy = [&](int img_block_idx, XFLOAT* corr_div_2, XFLOAT* fcoor_xy, XFLOAT* img_real_imag) {
		#pragma unroll 
		for (int i = tid; i < TParams::kImgBlockSize; i += TParams::kBlockSize) {
			...
			s_trans_pow2_accumulator[tid] += (img_real_imag[i * 2 + 0] * img_real_imag[i * 2 + 0] + 
											  img_real_imag[i * 2 + 1] * img_real_imag[i * 2 + 1])
											  * corr_div_2[i];
		}
	};
    ...
    auto init_fragment_c = [&] () {
		...
		if ((scheduler.get_strategy() == CoarseSchedulerStrategy::Default || 
		   	 scheduler.get_strategy() == CoarseSchedulerStrategy::InterleavedSplitK) && g_diff2s != g_diff2s_opt) {		
			#pragma unroll
		...
		// else, read from g_diff2s
				  ...
				  scheduler.get_strategy() == CoarseSchedulerStrategy::InterleavedSplitK)
				 && g_diff2s == g_diff2s_opt) {
			#pragma unroll
			for (int i = 0; i < kNumMmaTransInWarpTile; ++i) {
				#pragma unroll
				for (int j = 0; j < kNumMmaOrientInWarpTile; ++j) {
    ...
    while (scheduler.has_work()) {
        ...
        for (int i = tid; i < 2 * 2 * TParams::kTransBlockSize * TParams::kImgBlockSize; i += TParams::kBlockSize) {
            s_trans_mat_block[i] = 0.0;
        }

		// DEBUG
		// for (int i = tid; i < 2 * TParams::kTransBlockSize * TParams::kImgBlockSize; i += TParams::kBlockSize) {
        //     s_trans_mat_block_bak[i] = 0.0;
        // }
		...
		s_trans_mat_buffer[0] = 0;
		s_trans_mat_buffer[1] = 0;

		uint32_t* bitmap_buf = scheduler.get_bitmap_buf_from_workspace(work_space);
		DeviceBitmap device_bitmap(bitmap_buf, scheduler.get_K_block_num());
        __syncthreads();
...
/*=============================== FOR IMAGE BLOCK ==============================*/
        ...
        while (scheduler.get_current_work_next_k_cycle(k_cycle)) {
			...
			// trans fetch 
			if ((! scheduler.k_cycle_need_store_trans_mat(k_cycle + 1)) &&
				 k_cycle < scheduler.get_current_work_k_cycle_end() - 1 &&
				 k_cycle > scheduler.get_current_work_k_cycle_start()) {
					assert(k_cycle >= 0 && k_cycle < scheduler.current_work_k_num_); 
				int img_block_idx = scheduler.get_k_block_offset_from_k_cycle(k_cycle + 1);
				int k_cycle_next_mod2 = scheduler.k_cycle_mod<2>(k_cycle + 1);

				XFLOAT* trans_mat_buf = scheduler.get_trans_mat_buf_from_workspace(
					work_space, 
					img_block_idx / TParams::kImgBlockSize
				);

				translation_matrix_handler.async_load_translation_matrix_from_global_to_shared(
					s_trans_mat_block_swizzle[k_cycle_next_mod2],
					trans_mat_buf,
					device_bitmap,
					img_block_idx / TParams::kImgBlockSize,
					k_cycle_next_mod2,
			...
			bool need_construct_trans_mat = true;

			// trans sync 
			if ((! scheduler.k_cycle_need_store_trans_mat(k_cycle)) &&
				 k_cycle < scheduler.get_current_work_k_cycle_end() &&
				 k_cycle > scheduler.get_current_work_k_cycle_start() + 1) {
					assert(k_cycle >= 0 && k_cycle < scheduler.current_work_k_num_); 
				...
				if (k_cycle == scheduler.get_current_work_k_cycle_end() - 1) {
					need_construct_trans_mat = translation_matrix_handler.sync_and_check_bitmap(
						s_trans_mat_block_swizzle[k_cycle_mod2],
						device_bitmap,
						img_block_idx / TParams::kImgBlockSize,
						k_cycle_mod2,
						tid,
					...
					need_construct_trans_mat = !need_construct_trans_mat;
				} else {
					need_construct_trans_mat = translation_matrix_handler.sync_and_check_bitmap(
						s_trans_mat_block_swizzle[k_cycle_mod2],
						device_bitmap,
			...
			if (k_cycle > scheduler.get_current_work_k_cycle_start() && need_construct_trans_mat) {
				assert(k_cycle >= 0 && k_cycle < scheduler.current_work_k_num_); 
				int img_block_idx = scheduler.get_k_block_offset_from_k_cycle(k_cycle);
				int k_cycle_mod2 = scheduler.k_cycle_mod<2>(k_cycle);
				translation_matrix_handler.construct_translation_matrix_bank_conflict_free(
					s_trans_mat_block_swizzle[k_cycle_mod2],
					s_trans_real_mat_block_swizzle[k_cycle_mod2],
					s_trans_imag_mat_block_swizzle[k_cycle_mod2],
					s_trans_xy,
			...
			if (scheduler.k_cycle_need_store_trans_mat(k_cycle)) {
				...
				translation_matrix_handler.store_translation_matrix_to_global(
					trans_mat_buf,
					device_bitmap,
					img_block_idx / TParams::kImgBlockSize,
					s_trans_mat_block_swizzle[k_cycle_mod2],
			...
			if (k_cycle > scheduler.get_current_work_k_cycle_start() && 
				...
				TParams::kWarpTransTileSize, TParams::kWarpOrientTileSize, 2 * TParams::kWarpImgTileSize,
				TParams::kMmaTransTileSize, TParams::kMmaOrientTileSize, TParams::kMmaImgTileSize>(
					fragment_c, s_trans_mat_block_swizzle[k_cycle_mod2], s_orient_mat_block_swizzle, warp_id, lane_id);
			}
		...
		for (int i = TParams::kBlockSize / 2; i >= 1; i /= 2) {
			if (tid < i) {
        ...
        for (int i = 0; i < kNumMmaTransInWarpTile; ++i) {
            ...
            for (int j = 0; j < kNumMmaOrientInWarpTile; ++j) {
                ...
                for (int k = 0; k < kFragmentCSize; ++k) {
					...
					//  All translation share the same trans_pow2_accumulator placed at index 0.
                    fragment_c[i][j][k] += s_trans_pow2_accumulator[0] + s_orient_pow2_accumulator[n];
                }
            }
        }
        __syncthreads();
...
#endif //COARSE_MATRIX_KERNEL_IM2COL_SPLITIMG_BCF_PMO_TMDB_NFUSE_CUH