...
public:
	...
	AccProjectorKernel():
		mdlX(0), mdlXY(0), mdlZ(0),
		imgX(0), imgY(0), imgZ(0),
		mdlInitY(0), mdlInitZ(0),
		padding_factor(0),
		maxR(0), maxR2(0), maxR2_padded(0),
		mdlReal(0), mdlImag(0)
	{};
...
real = no_tex3D(mdlReal, xp, yp, zp, mdlX, mdlXY, mdlInitY, mdlInitZ);
	...
	void project3DmodelDouble(
			...
			double &imag)
	{
		double xp = (e0 * x + e1 * y ) * padding_factor;
		double yp = (e3 * x + e4 * y ) * padding_factor;
		double zp = (e6 * x + e7 * y ) * padding_factor;
...
real = no_tex3D(mdlReal, xp, yp, zp, mdlX, mdlXY, mdlInitY, mdlInitZ);
			...
			if (xp < 0)
			{
				...
				real =    tex3D<XFLOAT>(mdlReal, (float)(xp + 0.5), (float)(yp + 0.5), (float)(zp + 0.5));
				imag =  - tex3D<XFLOAT>(mdlImag, (float)(xp + 0.5), (float)(yp + 0.5), (float)(zp + 0.5));
			}
			else
			{
				yp -= mdlInitY;
